...
use XLite\Module\Amazon\PayWithAmazon\AMZ;

 class Order extends \XLite\Module\CDev\Coupons\Controller\Admin\Order implements \XLite\Base\IDecorator
{
    protected function getOrderDetails()
    {
        ...
        if ($order && $order->getDetails()) {
            foreach ($order->getDetails() as $detail) {
                $result[$detail->getName()] = $detail->getValue();
            }
        }
    ...
    public function handleRequest()
    {
        ...
        switch (\XLite\Core\Request::getInstance()->action) {
            case 'amz_capture':
                unset(\XLite\Core\Request::getInstance()->action);
                $amz_captured = false;

                $res = AMZ::func_amazon_pa_request('Capture', [
                    ...
                    'CaptureReferenceId'         => 'capture_' . \XLite\Core\Config::getInstance()->Amazon->PayWithAmazon->amazon_pa_order_prefix . '_' . $orderid,
                    'SellerCaptureNote'          => '',
                ]);
                if ($res) {
            ...
            case 'amz_void':
                ...
                $res = AMZ::func_amazon_pa_request('CloseAuthorization', [
                    'AmazonAuthorizationId' => $extra['amazon_pa_auth_id'],
                    'ClosureReason'         => '',
                ]);
            ...
            case 'amz_refund':
                ...
                $res = AMZ::func_amazon_pa_request('Refund', [
                    ...
                    'RefundReferenceId'         => 'refund_' . \XLite\Core\Config::getInstance()->Amazon->PayWithAmazon->amazon_pa_order_prefix . '_' . $orderid,
                    'SellerRefundNote'          => '',
                ]);
                if ($res) {
                    ...
                    if ($_ref_details) {
                        ...
                        $amz_refunded = in_array($_reply_status, ['Completed', 'Pending'], true);
            ...
            case 'amz_refresh':
                ...
                $res = AMZ::func_amazon_pa_request('GetAuthorizationDetails', [
                    'AmazonAuthorizationId' => $extra['amazon_pa_auth_id'],
                ]);
                ...
                if ($_auth_details) {
                    ...
                    $_oid = str_replace('auth_' . \XLite\Core\Config::getInstance()->Amazon->PayWithAmazon->amazon_pa_order_prefix . '_', '', $_auth_details['AuthorizationReferenceId'][0]['#']);
            ...
            case 'amz_refresh_refund_status':
                ...
                $res = AMZ::func_amazon_pa_request('GetRefundDetails', [
                ...
                if ($_ref_details) {
                    $amz_ref_id = $_ref_details['AmazonRefundId'][0]['#'];
                    $_reply_status = AMZ::func_array_path($_ref_details, 'RefundStatus/State/0/#');
                    $_reply_reason = AMZ::func_array_path($_ref_details, 'RefundStatus/ReasonCode/0/#');
                    $_oid = str_replace('refund_' . \XLite\Core\Config::getInstance()->Amazon->PayWithAmazon->amazon_pa_order_prefix . '_', '', $_ref_details['RefundReferenceId'][0]['#']);
