...
function isBootstrapUse(){return'undefined'!=typeof(jQuery.fn.modal)&&_.isFunction(jQuery.fn.modal)};var statesList=[],stateSelectors=[]
function UpdateStatesList(base){var _stateSelectors;base=base||document;jQuery('.country-selector',base).each(function(index,elem){statesList=array_merge(statesList,core.getCommentedData(elem,'statesList'));_stateSelectors=core.getCommentedData(elem,'stateSelectors');stateSelectors[_stateSelectors.fieldId]=new StateSelector(_stateSelectors.fieldId,_stateSelectors.stateSelectorId,_stateSelectors.stateInputId)})}
